...
<p><span>地址:</span> <span>${station.Address || station.address || '未知地址'}</span></p>
<p><span>可用自行车:</span> <span>${station.Available_bikes || station.available_bikes || '0'}</span></p>
<p><span>空位数量:</span> <span>${station.Available_bike_stands || station.available_bike_stands || '0'}</span></p>
<p><span>总容量:</span> <span>${station.Bike_stands || station.bike_stands || totalCapacity || '0'}</span></p>
`;
...
// 创建起点和终点位置
const startPos = { lat: parseFloat(startStation.Latitude), lng: parseFloat(startStation.Longitude) };
const endPos = { lat: parseFloat(endStation.Latitude), lng: parseFloat(endStation.Longitude) };
...
// 查找起点和终点站点信息
const startStation = window.stationsData.find(s => s.Name === document.getElementById('start-location').value);
const endStation = window.stationsData.find(s => s.Name === document.getElementById('end-location').value);
...
<p>可用车辆: ${startStation.Available_bikes || Math.floor(Math.random() * 10) + 5}</p>
</div>
<div class="detail-item">
<h4>终点站</h4>
<p>${endStation.Name}</p>
<p>可用停车位: ${endStation.Available_bike_stands || Math.floor(Math.random() * 15) + 5}</p>
</div>
`;
}
}
