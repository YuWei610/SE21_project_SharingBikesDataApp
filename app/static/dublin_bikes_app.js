  ...
  function initMapWithoutAPI() {
    ...
    mapElement.innerHTML = '<div style="width:100%;height:100%;display:flex;justify-content:center;align-items:center;background-color:#f0f0f0;"><p>地图加载失败，但您仍可以使用站点列表功能</p></div>';
    
    // 初始化全局对象以避免错误
    window.map = { getCenter: () => ({ lat: 53.349805, lng: -6.260310 }) };
    window.google = {
        maps: {
            Map: function() { return window.map; },
            Marker: function() { return { setMap: () => {}, getPosition: () => ({ lat: () => 53.349805, lng: () => -6.260310 }) }; },
            InfoWindow: function() { return { open: () => {} }; },
            LatLngBounds: function() { return { extend: () => {}, getCenter: () => ({ lat: 53.349805, lng: -6.260310 }) }; },
            SymbolPath: { CIRCLE: 0 },
            event: { addListener: () => {} }
        }
    };
...
function initMap() {
    try {
        // 基本地图初始化
        var location = { lat: 53.349805, lng: -6.260310 };  // 都柏林中心位置
...
function showStationInfoInSidebar(station) {
  if (!station.Available_bikes) station.Available_bikes = Math.floor(Math.random() * 10) + 5;
  if (!station.Available_bike_stands) station.Available_bike_stands = Math.floor(Math.random() * 15) + 5;
  const totalCapacity = parseInt(station.Available_bikes) + parseInt(station.Available_bike_stands);
  ...
  content.innerHTML = `
    <p><span>站点编号:</span> <span>${station.Number || station.number || 'N/A'}</span></p>
    <p><span>地址:</span> <span>${station.Address || station.address || 'Mock address ' + (station.Number || station.number || '')}</span></p>
    <p><span>可用自行车:</span> <span>${station.Available_bikes || station.available_bikes || '0'} 辆</span></p>
    <p><span>空位数量:</span> <span>${station.Available_bike_stands || station.available_bike_stands || '0'} 个</span></p>
    <p><span>总容量:</span> <span>${station.Bike_stands || station.bike_stands || totalCapacity || '0'} 辆</span></p>
  `;
...
function planJourney() {
  ...
  if (journeyPlannerPopup && journeyPlannerPopup.style.display === 'block') {
      ...
      startLocation = document.querySelector('#journey-planner-popup #start-location').value;
      endLocation = document.querySelector('#journey-planner-popup #end-location').value;
  } else {
      // 从侧边栏获取值
  ...
  const selectedDay = document.querySelector('input[name="journey-date"]:checked')?.value || 'today';
  const selectedTime = document.getElementById('journey-time')?.value || '00:05';
  ...
  // 显示加载状态
  const planButton = document.getElementById('plan-journey-btn') || document.querySelector('.btn');
  if (planButton) {
          ...
          fetch('http://localhost:5002/get_stations')
              ...
              .catch(error => {
                  ...
                  if (window.stationMarkers && window.stationMarkers.length > 0) {
                      ...
                      if (mockStations.length === 0) {
                          ...
                          for (let i = 1; i <= 20; i++) {
                              mockData.push({
                                  ...
                                  Longitude: -6.260310 + (Math.random() - 0.5) * 0.02,
                                  Available_bikes: Math.floor(Math.random() * 10) + 1,
                                  Available_bike_stands: Math.floor(Math.random() * 10) + 1
                              });
                          }
                      ...
                      // 查找匹配的站点
                      const startStation = findStationByNameOrAddress(window.stations, startLocation);
                      const endStation = findStationByNameOrAddress(window.stations, endLocation);
                      ...
                      const mockStart = {
                          ...
                          Longitude: -6.260310 + (Math.random() - 0.5) * 0.01,
                          Available_bikes: Math.floor(Math.random() * 10) + 1,
                          Available_bike_stands: Math.floor(Math.random() * 10) + 1
                      };
...
function calculateRoute(startStation, endStation) {
  ...
  if (document.getElementById('journey-planner-popup').style.display === 'block') {
      closeJourneyPlannerPopup();
  }
}
...
function showRouteDetails(startStation, endStation, distance, duration) {
  ...
  content.innerHTML = `
      ...
      <hr>
      <div class="detail-item">
          <strong>步行至起点站:</strong> ${walkDistanceStart.toFixed(2)} 公里 (约 ${walkTimeStart} 分钟)
      </div>
      <div class="detail-item">
          <strong>骑行:</strong> ${bikeDistance.toFixed(2)} 公里 (约 ${bikeTime} 分钟)
      </div>
      <div class="detail-item">
          <strong>步行至目的地:</strong> ${walkDistanceEnd.toFixed(2)} 公里 (约 ${walkTimeEnd} 分钟)
      </div>
      <hr>
      <div class="detail-item">
          <strong>起点站:</strong> ${startStation.Name || startStation.name}
          <div>可用自行车: ${startStation.Available_bikes || startStation.available_bikes || 5} 辆</div>
      </div>
      <div class="detail-item">
          <strong>终点站:</strong> ${endStation.Name || endStation.name}
          <div>可用停车位: ${endStation.Available_bike_stands || endStation.available_bike_stands || 8} 个</div>
      </div>
  `;
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // 添加事件监听器到行程规划按钮
  document.getElementById('journey-planner-btn').addEventListener('click', showJourneyPlannerPopup);
...
function renderCharts(bikeData, standData, hours, stationId) {
  ...
  // 渲染自行车可用性图表
  renderChart(bikeChartContainer, bikeData, hours, `站点 ${stationId} 自行车可用量`, 'bike');
  
  // 渲染停车位可用性图表
  renderChart(standChartContainer, standData, hours, `站点 ${stationId} 停车位可用量`, 'stand');
}
...
function applyFilters() {
    ...
    if (!stationId) {
        ...
        resultsContent.querySelector('.results-content').innerHTML = '<p class="error-text">Please select a station</p>';
        return;
    }
    
    // 显示加载中提示
    const resultsContent = document.getElementById('filter-results');
    resultsContent.querySelector('.results-content').innerHTML = '<p style="text-align:center">Loading...</p>';
    ...
    .then(data => {
        // 处理筛选结果
        const resultsContent = document.getElementById('filter-results').querySelector('.results-content');
    ...
    .catch(error => {
        ...
        const resultsContent = document.getElementById('filter-results').querySelector('.results-content');
        resultsContent.innerHTML = '<p class="error-text">Error getting data, please try again</p>';
    });
}
...
function handleSubmitJourney() {
    // 获取表单数据
    var journeyDate = document.querySelector('input[name="journey-date"]:checked').value;
    ...
    console.log("规划行程: ", {journeyDate, journeyTime, startLocation, endLocation});
    ...
    routeInfoContent.innerHTML = `
        ...
        <div class="detail-item">
            <strong>计划日期:</strong> 
            ${journeyDate === 'today' ? '今天' : journeyDate === 'tomorrow' ? '明天' : '后天'}
        </div>
        <div class="detail-item">
            <strong>计划时间:</strong> 
            ${journeyTime}
...
function handleDirections() {
    ...
    if (window.google && window.google.maps && window.google.maps.DirectionsService) {
        ...
        var routeInfoContent = document.getElementById("route-info-content");
        routeDetails.style.display = "block";
        routeInfoContent.innerHTML = '<div style="text-align:center;padding:20px;">正在获取路线导航信息...</div>';
        ...
        directionsService.route(request, function(result, status) {
            if (status == google.maps.DirectionsStatus.OK) {
                ...
                document.getElementById("open-google-maps").addEventListener("click", function() {
                    var mapsUrl = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(startLocation)}&destination=${encodeURIComponent(endLocation)}&travelmode=bicycling`;
...
function displayFilterResults(data) {
    ...
    if (data.results[0].hasOwnProperty('predicted_bikes')) {
        columns.push(
            { id: 'predicted_bikes', label: `预测车辆 (${data.results[0].prediction_time})` },
            { id: 'predicted_stands', label: `预测车位 (${data.results[0].prediction_time})` }
        );
    }
            ...
            if (column.id === 'usage_percentage') {
                ...
                cell.innerHTML = `
                    <div class="progress-bar">
                        <div class="progress" style="width: ${percentage}%; background-color: ${getColorForPercentage(percentage)}"></div>
...
function resetFilters() {
    ...
    // 清空结果区域
    const resultsContent = document.getElementById('filter-results').querySelector('.results-content');
    resultsContent.innerHTML = '<p class="placeholder-text">申请筛选后将显示结果...</p>';
}
